 ...
 *        - 用 costmap 衍生的 obs_min + 基于当前 pose 与已保存全局路径计算的 target(3) + last_action（2，上一次模型输出）
...
{
// 配置函数，自己不触发，保存节点指针和 costmap 指针，加载参数
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  // 从参数服务器读入参数，保存在第二个输入变量中（这里为类成员），第三个输入变量是默认值
...
// 重置函数（不知道跟清理函数有什么区别），自己不触发，清空所有历史观测
...
// 获得全局规划的路径并进行稀疏化处理，自己不调用（官方插件也是如此）
...
// 路径稀疏化，每 distance_threshold 米保留一个路径点
...
// 计算一次速度控制指令（注释输入参数代表不需要，同时避免未使用警告）
...
// 组装历史观测为模型输入
std::vector<float> RLController::assembleObservation(
  const geometry_msgs::msg::PoseStamped * pose,
  const geometry_msgs::msg::Twist * /*vel*/,
  std::vector<float> & current_frame_out)
{
  // 从 history_frames_中按照时间从旧到新取出最多 history_length_ 帧（每帧 25 维），不足则用当前帧填充（？）
  // 组装输入：按照时间从旧到新拼接 history_length_ 帧历史（已包含模型输出）并在最后追加当前帧（不包含本次模型输出，本次输出是放在下一帧里的）
  ...
  // obs20: 基于 costmap 扇区化计算（始终本地计算，不从外部计算并订阅）
...
// 构造一帧输入（25维）
std::vector<float> RLController::computeFallbackObservation(const geometry_msgs::msg::PoseStamped * pose)
{
  // 生成一帧默认 25 维观测
  // 前 20 位使用基于 costmap 的扇区化距离（如果不可用则返回 0 表示最近），随后 3 个 target，2 个 last_action
  std::vector<float> obs(obs_dim_, 0.0f);

  // 使用 costmap 扇区化观测（或在无 costmap 时使用默认值）
  std::vector<float> obs20 = computeObsFromCostmap(pose);
  for (size_t i = 0; i < min_obs_dim_ && i < obs.size(); ++i) {
      obs[i] = obs20[i];
  }

  // 计算到目标的sin、cos、distance
  double target_cos = 0.0, target_sin = 0.0, target_dist = 0.0;
  if (pose && have_plan_) {
      std::tie(target_cos, target_sin, target_dist) = computeTargetFromPlan(*pose);
  }
  obs[min_obs_dim_ + 0] = static_cast<float>(target_cos);
  obs[min_obs_dim_ + 1] = static_cast<float>(target_sin);
  obs[min_obs_dim_ + 2] = static_cast<float>(target_dist);
...
std::vector<float> RLController::computeObsFromCostmap(const geometry_msgs::msg::PoseStamped * pose)
{
  ...
  for (size_t i = 0; i < min_obs_dim_; ++i) {
    ...
    for (size_t r = 0; r < std::max<size_t>(1, rays_per_sector_); ++r) {
      ...
      if (found_dist < sector_min)
        sector_min = found_dist;
...
std::vector<float> RLController::runModel(const std::vector<float> & input)
{
  ...
  try {
    ...
    // Note: GetInputName/GetOutputName allocate with allocator; free by letting allocator out of scope
