	...
	std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
	...
	if (node)
		RCLCPP_INFO(node->get_logger(), "RLController::configure done (%s)", plugin_name_.c_str());
	...
	try {
		...
		if (!ort_env_ && !ort_failed_) {
			...
			if (node)
				RCLCPP_INFO(node->get_logger(), "Ort::Env initialized in configure for %s", plugin_name_.c_str());
		}
	} catch (const std::exception & e) {
		if (node)
			RCLCPP_ERROR(node->get_logger(), "Failed to initialize Ort::Env in configure: %s", e.what());
		ort_failed_ = true;
	} catch (...) {
		if (node)
			RCLCPP_ERROR(node->get_logger(), "Unknown failure initializing Ort::Env in configure");
		ort_failed_ = true;
	}
}
	...
	if (filled < history_length_) {
		...
		for (size_t i = 0; i < filled; ++i) {
			...
			if (frame.size() != obs_dim_) {
				auto node = node_.lock();
				if (node)
					RCLCPP_WARN(node->get_logger(), "Skipping malformed history frame %zu (size %zu != %zu)", i, frame.size(), obs_dim_);
				for (size_t j = 0; j < obs_dim_ && idx < input.size(); ++j) {
					input[idx++] = current_frame[j];
				}
				continue;
